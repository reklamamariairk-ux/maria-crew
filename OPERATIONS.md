...
1. Открой https://crew.145-223-121-47.sslip.io/api/health → должно быть `{ ok: true }`
...
3. Проверь webhook руками:
   ...
   ```
   Должно показать `url: "https://crew.145-223-121-47.sslip.io/webhook/<secret>"` и `pending_update_count: 0`.
...
**Что делать:**
1. Локально воспроизведи: `npm test` — найдёшь упавший assert
2. Поправь либо логику, либо тест (если требование изменилось)
...
- `GET https://crew.145-223-121-47.sslip.io/api/health` — должно быть `{ ok: true, ... }`
...
2. Добавь монитор:
   - Type: **HTTP(s)**
   - URL: `https://crew.145-223-121-47.sslip.io/api/health/detailed`
   - Interval: **5 minutes**
...
```bash
curl https://crew.145-223-121-47.sslip.io/api/health/detailed | jq '.checks.crons'
```
