...
router.put('/employees/:employeeId', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    // Гарантируем наличие записи в monthly_metrics
    await pool.query(
    ...
    // Если isMvp=true — снимаем MVP с остальных в той же точке/месяце
    if (isMvp === true) {
      await pool.query(
...
router.put('/stores/:storeId', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    // Если isTop=true — снимаем ТОП с остальных
    if (isTop === true) {
      await pool.query(
    ...
    // Перерасчёт rank по убыванию total_score
    await pool.query(
      `WITH ranked AS (
