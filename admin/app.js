...
async function setEmployeeMvp(employeeId, storeId) {
  ...
  if (!await confirmDialog({
    ...
    message: 'Лучших на точке может быть несколько — статусы остальных не изменятся. При «Обработать месяц» каждый лучший получит особую карточку и торт или пирог «Мария».',
...
async function unsetEmployeeMvp(employeeId, storeId) {
  ...
  if (!await confirmDialog({
    ...
    warning: 'Уже начисленные монеты, выданная особая карточка и назначенный торт НЕ возвращаются — при необходимости откати их вручную (торт — в секции «Торты месяца»).',
    confirmText: 'Снять',
    danger: true,
  })) return;
  try {
